...
function Invoke-Unlock {
  ...
  try {
    if (-not (Test-RingActive)) {
      try { Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'override_v6_profile' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue } } catch {}
...
$script:EDGE_PROFILE = Join-Path $env:TEMP 'override_v6_profile'
function Find-Edge {
...
function Test-QuizPresent {
  ...
  try {
    ...
    if ($script:rg_useEdge) {
      return (@(Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'override_v6_profile' }).Count -gt 0)
    } else {
...
function Stop-QuizProcs {
  ...
  try { Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 |
        Where-Object { $_.CommandLine -match 'override_v6_profile' } |
...
function Resolve-Sounds {
  ...
  if ((@($t1).Count + @($t2).Count + @($t3).Count) -eq 0) {
    $fb = Join-Path $env:TEMP 'override_v6_fallback.wav'
...
function Remove-Alarms {
  ...
  Get-ScheduledTask -ErrorAction SilentlyContinue |
    Where-Object { ($_.TaskName -like 'OVERRIDE_V6_*' -or $_.TaskName -like 'OVERRIDE_V5_*' -or $_.TaskName -like 'OVERRIDE_V4_*' -or $_.TaskName -like 'OVERRIDE_V3_*' -or $_.TaskName -like 'OVERRIDE_V2_*') -and $_.TaskName -notlike 'OVERRIDE_LIVE*' } |
...
function Register-Alarms {
  ...
  Write-Host "$n alarm(s) armed (v6). Legacy OVERRIDE_V5_*/V4_*/V3_*/V2_* tasks were replaced." -ForegroundColor Green
}
function Get-ArmedCount {
  $n = 0
  foreach ($t in @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V6_*' -and $_.TaskName -notlike '*_safe_*' })) {
    try { $i = $t | Get-ScheduledTaskInfo; if ($i.NextRunTime -and $i.NextRunTime -gt (Get-Date)) { $n++ } } catch {}
  }
  $n
}
function Show-Tasks {
  $tasks = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V6_*' -and $_.TaskName -notlike '*_safe_*' })
  if ($tasks.Count -eq 0) { Write-Host "   (no v4 alarms armed)" -ForegroundColor DarkYellow; return }
  foreach ($t in ($tasks | Sort-Object TaskName)) { $i = $t | Get-ScheduledTaskInfo; Write-Host ("    {0,-10} next: {1,-22} [{2}]" -f ($t.TaskName -replace '^OVERRIDE_V6_',''), $i.NextRunTime, $t.State) -ForegroundColor Green }
}
...
function Import-PriorConfig {
  ...
  $up = Split-Path -Parent $script:root
  foreach ($prior in @((Join-Path $up 'v5\config.json'), (Join-Path $up 'v4\config.json'), (Join-Path $up 'v3\config.json'), (Join-Path $up 'v2\config.json'))) {
...
if ($Disarm) { Write-Host "OVERRIDE v6 // disarming..." -ForegroundColor Yellow; Remove-Alarms; return }
if ($Arm)    { Write-Host "OVERRIDE v6 // arming..." -ForegroundColor Green; Register-Alarms; return }
