...
4. **Lockdown always releases — and is time-capped.** The keyboard hook is a per-process
   static, so the +6min safe task CANNOT remove a *live* ring's hook (it runs in another
   process — see bug museum #20). The real guarantees: (a) the ring drops its OWN hook after
   `lockdownMaxMin` (default 6) even if the alarm keeps ringing; (b) every graceful exit runs
   the `finally` (`[Lockdown]::Remove()`); (c) a `WH_KEYBOARD_LL` hook is auto-removed by Windows
   when the process dies, so a hard-kill also frees the keyboard; (d) the safe task's `-Unlock`
   re-enables Task Manager (registry, cross-process OK) and reaps orphaned quiz windows when no
   ring is active (#21); (e) `Ctrl+Alt+Del` is never blockable and always works.
...
**Reorder (v6):** each alarm row has ▲/▼ buttons — `Panel-MoveAlarm $id $dir` swaps with the
...
Session files live in the **v6 root** (`quiz.hta` computes `parent(dir(self))`).
...
| 16 | panel dumped in a corner / bottom of screen | TWO stacked causes: (a) the quiz.hta rewrite dropped the `X-UA-Compatible IE=edge` meta → quirks mode (auto-margins don't centre, viewport sizes unreliable); (b) IE11 ignores `transform` and collapses injected `table-cell`s, so CSS-only vertical centring is hopeless in mshta | the meta is back and marked LOAD-BEARING in quiz.hta; vertical centring is JS (`centerPanel()` in core.js: `position:relative;top` — NOT margin-top, which collapses through #stage), re-run on new question/victory/resize. Verified via getBoundingClientRect: left 508=(1898−880)/2, top 337=(1080−405)/2 |
| **17** | **(SEVERE, real-world, v3+v4) alarm window closed + reopened every ~0.5s for 3 min — unsolvable, forced 3 hard shutdowns** | the ring decided "is the quiz alive?" from the **launched process's `HasExited`**. mshta AND Edge both hand the window off to a *different* process, so the launched PID exits within ~1s while the window lives on → engine thinks it died → relaunches → repeat every tick. **Never caught in tests because test mode sets `Relaunch=$false`.** | (a) **launch grace**: after any launch, do not evaluate relaunch for `rg_launchGrace`=12s (caps relaunch to ≤1 per 12s — can't thrash, and 12s is enough to read+solve). (b) **`Test-QuizPresent`**: truth comes from process presence (CIM cmdline match for mshta `quiz` / Edge `override_v4_profile`) or recent listener beats — NOT the launched PID; uncertain → assume alive. (c) relaunch checked ≤every 2s, each relaunch re-anchors the grace. Test via `session.testcfg {"relaunch":true,"quiet":true}`. Verified: PID stable t=6→28s across the 12s boundary. |
| **18** | **(v4 browser only) Edge kiosk self-closed at ~12s, then slow-relaunched** | the browser heartbeat is an `<img src=/beat>` beacon that resets its failure counter on `onload`; the listener replied with the literal string `"GIF89a"`, which is **not a decodable image** → `onload` never fired → `BEAT_FAILS` climbed → at the 12s startup-grace the quiz concluded "engine gone" and called `window.close()` | the listener now returns a **real 43-byte 1×1 GIF** (`$script:rg_gif`) so the beacon decodes and `onload` fires. (mshta/`quiz.hta` is unaffected — it uses the `session.beat` *file* freshness, not the beacon.) Verified: kiosk PID stable t=7→28s. |
| **19** | **(v4 panel, user-reported) ~1-2s lag after every panel action + matrix rain froze** | `Panel-Persist` re-registered ALL Windows scheduled tasks (`Register-Alarms`, ~1-2s) synchronously on the UI thread on every click/toggle — freezing the whole UI (and the rain with it). | save config INSTANTLY, **debounce** the slow arming: `Panel-ScheduleArm` (re)starts a 1.2s timer; arming runs once after you stop interacting, and is flushed on close if still pending. Also: `powercfg` wake-timer setup gated to once/process; panel rain 8→6fps. |
| **20** | **(v5 audit, HIGH lockout) a long alarm could keyboard-block hotkeys for up to 60 min** | the keyboard hook handle is a **per-process static**; the +6min `OVERRIDE_*_safe_*` task calls `[Lockdown]::Remove()` in a DIFFERENT process, which is a no-op against the live ring's hook. So the hook only released at the ring's own deadline (durationMin, up to 60). MAINTENANCE invariant #4 had wrongly claimed the safe task releases it. | the ring drops its OWN hook after `lockdownMaxMin` (default 6) via `rg_lockUntil` in `Ring-Tick`, while the alarm keeps ringing (sound keeps you engaged). Invariant #4 rewritten to match. `Ctrl+Alt+Del` was always available regardless. |
| **21** | **(v5 audit, HIGH stuck-window) hard-killing the ring orphans the fullscreen Edge/mshta** | `Stop-QuizProcs` runs only in Run-Ring's `finally`; a hard kill (Task Scheduler time limit, End Task, crash) skips it, leaving a fullscreen window with no engine. The quiz's own `engineGone` self-close is the only backstop (~12-20s, and not during the 12s startup grace). | `Invoke-Unlock` (the +6min safe task) now reaps `*override_v5_profile*` Edge + `quiz.hta` mshta — but ONLY when `Test-RingActive` says no ring holds the mutex, so a legitimately-running long alarm is never killed. |
