/* OVERRIDE v4 // shared quiz core — THEMED edition
   ES3/ES5-only JavaScript: runs in Edge/Chrome/Safari/Firefox (primary, kiosk mode),
   mshta IE11 (fallback shell) and cscript (selftest harness).
   ...
   - cinematic victory: panel dissolves into CJK -> ACCESS GRANTED types itself ->
     champion quote spoken aloud. The quote is sacred: do not change it.

   env contract (set by quiz.html / quiz.hta):
     label, numQuestions, difficulty, cats[], matrixRain, deadlineMs, theme, user,
     speak(text), unlock(), engineGone() -> bool, closeWin()
*/
...
var OVERRIDE_CORE = (function () {
  ...
  /* ---------------- generators (UNCHANGED from v3 — selftested) ---------------- */
...
var OVERRIDE_UI = (function () {
  ...
  /* ---- effect: RGB-split glitch burst (CSS does the tearing via .gl + data-text) ---- */
  function glitchBurst(id, ms) {
    try {
      var t = el(id); if (!t) return;
      t.setAttribute("data-text", t.innerText || t.textContent || "");
      t.className += " gl";
      setTimeout(function () { try { t.className = t.className.replace(/\s*\bgl\b/, ""); } catch (e) { } }, ms || 450);
    } catch (e) { }
  }
  /* ---- effect: red screen flash ---- */
  function redFlash() {
    try {
      var f = el('flash'); f.className = "on";
      setTimeout(function () { try { f.className = ""; } catch (e) { } }, 330);
    } catch (e) { }
  }
  ...
  function shake() {
    var p = el('panel');
    var keep = p.className.replace(/\s*\bshake\b/, "");
    p.className = keep;
    setTimeout(function () { p.className = keep + " shake"; }, 10);
    setTimeout(function () { p.className = keep; }, 460);
  }
  ...
  function check() {
    ...
    if (C.isHit(Q, box.value)) {
      ...
      speak(taunt);
      shake();
      glitchBurst('qtext', 480);
      glitchBurst('title', 480);
      redFlash();
  ...
  /* ---- the cinematic ending: CJK dissolve -> ACCESS GRANTED types itself ---- */
  function scramblePass(ids, ratio) {
    var i, j, t, src, out;
    for (i = 0; i < ids.length; i++) {
      t = el(ids[i]); if (!t) continue;
      src = t.innerText || t.textContent || "";
      out = "";
      for (j = 0; j < src.length; j++) {
        out += (src.charAt(j) === " ") ? " " : (Math.random() < ratio ? cjkChar() : src.charAt(j));
      }
      t.innerHTML = out;
    }
  }
  function victory() {
    ...
    speak(C.pick(C.LINES.victoryPre) + " " + quote, true);

    /* stage 1: the quiz dissolves into CJK and fades (the vanish you love) */
    glitchBurst('title', 400);
    var ids = ['title', 'subline', 'qcat', 'qtext', 'progtxt', 'msg', 'hint'];
    var pass = 0, panel = el('panel');
    var iv = setInterval(function () {
      pass++;
      scramblePass(ids, pass / 6);
      if (pass >= 6) {
        clearInterval(iv);
        panel.style.opacity = "0.06";
        setTimeout(stage2, 320);
      }
    }, 110);
    ...
    function stage2() {
      ...
      glitchBurst('granted', 500);
      centerPanel();
      /* the champion quote types itself */
      var qe = el('quote'), k = 0;
      var tv = setInterval(function () {
        k++;
        qe.innerHTML = quote.substring(0, k) + '<span class="blink">_</span>';
        if (k >= quote.length) {
          clearInterval(tv);
          qe.innerHTML = quote;
          ...
          el('stamp').innerHTML = "[ STATUS: DAY — WON ]";
          centerPanel();
        }
      }, 28);
    }
  ...
  function startRain() {
    var c = el('rain'), x, fs = 16, cols, drops = [], i,
      ...
      colors = RAIN_COLORS[env.theme] || RAIN_COLORS.green;
