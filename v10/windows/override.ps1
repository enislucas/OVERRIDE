...
# OVERRIDE v10 // WAKE PROTOCOL (UNIVERSAL: quiz core shared with v10/web) — Windows engine (THEMED edition, started as a frozen copy of v5)
...
$script:root = Split-Path -Parent $script:eng          # v6 root: session files + config live here
...
function Invoke-Unlock {
  ...
  try {
    if (-not (Test-RingActive)) {
      try { Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'override_v10_profile' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue } } catch {}
...
$script:EDGE_PROFILE = Join-Path $env:TEMP 'override_v10_profile'
function Find-Edge {
...
function Test-QuizPresent {
  ...
  try {
    ...
    if ($script:rg_useEdge) {
      return (@(Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'override_v10_profile' }).Count -gt 0)
    } else {
...
function Stop-QuizProcs {
  ...
  try { Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -OperationTimeoutSec 4 |
        Where-Object { $_.CommandLine -match 'override_v10_profile' } |
...
function Resolve-Sounds {
  ...
  if ((@($t1).Count + @($t2).Count + @($t3).Count) -eq 0) {
    $fb = Join-Path $env:TEMP 'override_v10_fallback.wav'
...
function Remove-Alarms {
  ...
  Get-ScheduledTask -ErrorAction SilentlyContinue |
    Where-Object { ($_.TaskName -like 'OVERRIDE_V10_*' -or $_.TaskName -like 'OVERRIDE_V6_*' -or $_.TaskName -like 'OVERRIDE_V5_*' -or $_.TaskName -like 'OVERRIDE_V4_*' -or $_.TaskName -like 'OVERRIDE_V3_*' -or $_.TaskName -like 'OVERRIDE_V2_*') -and $_.TaskName -notlike 'OVERRIDE_LIVE*' } |
...
function Get-ArmedCount {
  $n = 0
  foreach ($t in @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V10_*' -and $_.TaskName -notlike '*_safe_*' })) {
    try { $i = $t | Get-ScheduledTaskInfo; if ($i.NextRunTime -and $i.NextRunTime -gt (Get-Date)) { $n++ } } catch {}
  }
  $n
}
function Show-Tasks {
  $tasks = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V10_*' -and $_.TaskName -notlike '*_safe_*' })
  if ($tasks.Count -eq 0) { Write-Host "   (no v4 alarms armed)" -ForegroundColor DarkYellow; return }
  foreach ($t in ($tasks | Sort-Object TaskName)) { $i = $t | Get-ScheduledTaskInfo; Write-Host ("    {0,-10} next: {1,-22} [{2}]" -f ($t.TaskName -replace '^OVERRIDE_V10_',''), $i.NextRunTime, $t.State) -ForegroundColor Green }
}
...
function Import-PriorConfig {
  ...
  $up = Split-Path -Parent $script:root
  foreach ($prior in @((Join-Path $up 'v6\config.json'), (Join-Path $up 'v5\config.json'), (Join-Path $up 'v4\config.json'), (Join-Path $up 'v3\config.json'), (Join-Path $up 'v2\config.json'))) {
...
# move an alarm up (-1) or down (+1) in the list order, then persist + re-render (v6 feature).
# Order is cosmetic (alarms fire by their scheduled time regardless) — this just organizes the list.
function Panel-MoveAlarm($id, $dir) {
  $arr = @($script:pn_alarms)
  if ($arr.Count -lt 2) { return }
  $ids = @($arr | ForEach-Object { [string]$_.Id })
  $i = [array]::IndexOf([object[]]$ids, [string]$id)
  if ($i -lt 0) { return }
  $j = $i + $dir
  if ($j -lt 0 -or $j -ge $arr.Count) { return }          # already at the top/bottom
  $t = $arr[$i]; $arr[$i] = $arr[$j]; $arr[$j] = $t
  $script:pn_alarms = $arr
  Panel-Persist; Panel-RenderRows; Panel-RefreshArmed; Panel-UpdateStatus
  Panel-Log $(if ($dir -lt 0) { "moved up" } else { "moved down" })
}
...
function Show-PanelGui {
  ...
  $script:pn_form.Text = "OVERRIDE // CONTROL v10.7"; $script:pn_form.FormBorderStyle = 'Sizable'; $script:pn_form.MaximizeBox = $true
  ...
  $sub = New-Object System.Windows.Forms.Label; $sub.Text="WAKE PROTOCOL // v10.7"; $sub.Left=20; $sub.Top=52; $sub.Width=300; $sub.Height=18; $sub.ForeColor=$script:pn_pal.Dim; $sub.BackColor=[System.Drawing.Color]::Transparent; $sub.Font=New-Object System.Drawing.Font('Consolas',9); $script:pn_box.Controls.Add($sub)
...
if ($Disarm) { Write-Host "OVERRIDE v10 // disarming..." -ForegroundColor Yellow; Remove-Alarms; return }
if ($Arm)    { Write-Host "OVERRIDE v10 // arming..." -ForegroundColor Green; Register-Alarms; return }
...
# v10.7: the panel must survive however it was launched. If this process OWNS its console
# (double-click / .bat start), hide it - a visible console shows raw arm logs ("random
# PowerShell") and closing it kills the panel. If launched from a real terminal (shared
# console), leave it alone.
try {
  Add-Type -Namespace OvrWin -Name ConsoleUtil -MemberDefinition '[DllImport("kernel32.dll")] public static extern IntPtr GetConsoleWindow(); [DllImport("user32.dll")] public static extern bool ShowWindow(IntPtr h, int cmd); [DllImport("kernel32.dll")] public static extern uint GetConsoleProcessList(uint[] l, uint c);' -ErrorAction SilentlyContinue
  $cl = New-Object uint[] 8
  if ([OvrWin.ConsoleUtil]::GetConsoleProcessList($cl, 8) -le 1) {
    $h = [OvrWin.ConsoleUtil]::GetConsoleWindow()
    if ($h -ne [IntPtr]::Zero) { [OvrWin.ConsoleUtil]::ShowWindow($h, 0) | Out-Null }
  }
} catch {}
