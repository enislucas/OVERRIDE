...
(function () {
  ...
  /* vibration (Android; harmless no-op on iOS Safari, which has no navigator.vibrate) */
  var vibeTimer = null;
  ...
  function renderSetup() {
    ...
    brand.innerHTML = '<div class="logo">OVERRIDE</div><div class="tag">WAKE PROTOCOL // v8.1 - Android</div>';
    r.appendChild(brand);
    ...
    c4.innerHTML = '<h2>BEFORE YOU SLEEP - do these once (Samsung)</h2><ul class="check">' +
      '<li><span class="k">1.</span> Chrome menu (&#8942;) &gt; <b>Add to Home screen / Install app</b>, then open it from the new icon.</li>' +
      '<li><span class="k">2.</span> <b>Media volume UP</b> (press a volume key, tap the dropdown, raise the <i>Media</i> slider).</li>' +
      '<li><span class="k">3.</span> Settings &gt; Battery &gt; turn <b>OFF "Put unused apps to sleep"</b>, and remove OVERRIDE from <b>Sleeping / Deep sleeping apps</b>.</li>' +
      '<li><span class="k">4.</span> Settings &gt; Apps &gt; <b>OVERRIDE</b> &gt; Battery &gt; <b>Unrestricted</b>.</li>' +
      '<li><span class="k">5.</span> <b>Plug it in</b>; <b>Power saving OFF</b>; keep the <b>screen on</b>.</li>' +
      '<li><span class="k">6.</span> Tap <b>ARM</b>, leave it open, and <b>don&#39;t swipe it away from Recents</b>.</li>' +
      '<li><span class="k">7.</span> <b>Failsafe:</b> set a normal <b>Clock app alarm</b> at the same time too.</li></ul>';
    r.appendChild(c4);

    var note = document.createElement('div'); note.className = 'note';
    note.innerHTML = 'honest limit: a website can&#39;t lock the phone like a real app - it stays loud, vibrates &amp; is quiz-gated, but keep it open &amp; foreground.';
    r.appendChild(note);
  }
  ...
  function renderArmed() {
    ...
    cd.innerHTML =
      ...
      '<div class="sub"><span class="warn">Screen must stay ON (do not let it lock) &amp; the phone plugged in.</span><br>' +
      'If the ● above stops blinking, the OS froze the page - tap the screen to wake it.<br>' +
