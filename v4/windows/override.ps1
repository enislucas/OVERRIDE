...
# OVERRIDE v4 // WAKE PROTOCOL — Windows engine (THEMED edition)
...
$script:EDGE_PROFILE = Join-Path $env:TEMP 'override_v4_profile'
function Find-Edge {
...
function Stop-QuizProcs {
  ...
  try { Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" |
        Where-Object { $_.CommandLine -match 'override_v4_profile' } |
...
function Resolve-Sounds {
  ...
  if ((@($t1).Count + @($t2).Count + @($t3).Count) -eq 0) {
    $fb = Join-Path $env:TEMP 'override_v4_fallback.wav'
...
function Remove-Alarms {
  ...
  Get-ScheduledTask -ErrorAction SilentlyContinue |
    Where-Object { ($_.TaskName -like 'OVERRIDE_V4_*' -or $_.TaskName -like 'OVERRIDE_V3_*' -or $_.TaskName -like 'OVERRIDE_V2_*') -and $_.TaskName -notlike 'OVERRIDE_LIVE*' } |
...
function Register-Alarms {
  ...
  foreach ($a in $script:cfg.alarms) {
    ...
    Register-ScheduledTask -TaskName "OVERRIDE_V4_$($a.id)" -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Force | Out-Null
    ...
    Register-ScheduledTask -TaskName "OVERRIDE_V4_safe_$($a.id)" -Action $safeAction -Trigger $safeTrigger -Settings $safeSettings -Principal $principal -Force | Out-Null
    Write-Host "  armed  $($a.label)  $desc  [theme: $(Get-Prop $a 'theme' 'green')]" -ForegroundColor Green; $n++
  }
  Write-Host "$n alarm(s) armed (v4). Legacy OVERRIDE_V3_*/V2_* tasks were replaced." -ForegroundColor Green
}
function Get-ArmedCount {
  $n = 0
  foreach ($t in @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V4_*' -and $_.TaskName -notlike '*_safe_*' })) {
    try { $i = $t | Get-ScheduledTaskInfo; if ($i.NextRunTime -and $i.NextRunTime -gt (Get-Date)) { $n++ } } catch {}
  }
  $n
}
function Show-Tasks {
  $tasks = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -like 'OVERRIDE_V4_*' -and $_.TaskName -notlike '*_safe_*' })
  if ($tasks.Count -eq 0) { Write-Host "   (no v4 alarms armed)" -ForegroundColor DarkYellow; return }
  foreach ($t in ($tasks | Sort-Object TaskName)) { $i = $t | Get-ScheduledTaskInfo; Write-Host ("    {0,-10} next: {1,-22} [{2}]" -f ($t.TaskName -replace '^OVERRIDE_V4_',''), $i.NextRunTime, $t.State) -ForegroundColor Green }
}
...
function Import-PriorConfig {
  ...
  $up = Split-Path -Parent $script:root
  foreach ($prior in @((Join-Path $up 'v3\config.json'), (Join-Path $up 'v2\config.json'))) {
...
function Show-PanelGui {
  ...
  $script:pn_form.Text = "OVERRIDE // CONTROL v4"; $script:pn_form.FormBorderStyle = 'Sizable'; $script:pn_form.MaximizeBox = $true
